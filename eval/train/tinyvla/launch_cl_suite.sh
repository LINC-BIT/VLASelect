...
SUITE_ROOT="ckpt/tinyvla/cl_suite/${SUITE_STAMP}"
...
declare -A SCRIPT_BY_METHOD=(
    [conrft]="train/tinyvla/conrft/run_online_rl.sh"
    [convertnet]="train/tinyvla/convertnet/run_online_rl.sh"
    [edgeta]="train/tinyvla/edgeta/run_online_rl.sh"
    [flare]="train/tinyvla/flare/run_online_rl.sh"
    [improv_vla]="train/tinyvla/improv_vla/run_online_rl.sh"
    [ours]="train/tinyvla/ours/run_online_rl_cl.sh"
    [ppo_gen]="train/tinyvla/ppo_gen/run_online_rl.sh"
    [self_improv]="train/tinyvla/self_improv/run_online_rl.sh"
    [vla_rft]="train/tinyvla/vla_rft/run_online_rl.sh"
    [world_env]="train/tinyvla/world_env/run_online_rl.sh"
)

declare -A EXP_NAME_BY_METHOD=(
    [conrft]="tinyvla/cl_suite/${SUITE_STAMP}/conrft"
    [convertnet]="tinyvla/cl_suite/${SUITE_STAMP}/convertnet"
    [edgeta]="tinyvla/cl_suite/${SUITE_STAMP}/edgeta"
    [flare]="tinyvla/cl_suite/${SUITE_STAMP}/flare"
    [improv_vla]="tinyvla/cl_suite/${SUITE_STAMP}/improv_vla"
    [ours]="tinyvla/cl_suite/${SUITE_STAMP}/ours"
    [ppo_gen]="tinyvla/cl_suite/${SUITE_STAMP}/ppo_gen"
    [self_improv]="tinyvla/cl_suite/${SUITE_STAMP}/self_improv"
    [vla_rft]="tinyvla/cl_suite/${SUITE_STAMP}/vla_rft"
    [world_env]="tinyvla/cl_suite/${SUITE_STAMP}/world_env"
)
...
resolve_suite_root() {
    ...
    elif [[ -d "$ROOT_DIR/ckpt/tinyvla/cl_suite/$value" ]]; then
        candidate="$ROOT_DIR/ckpt/tinyvla/cl_suite/$value"
    else
        return 1
    fi
    (
...
for method in "${METHOD_ORDER[@]}"; do
    if [[ "${SHOULD_RUN[$method]}" != "1" && -z "$INHERITED_SUITE_ROOT" ]]; then
        continue
    fi
    ...
    if [[ ! -f "$script_path" ]]; then
        echo "Missing script for method $method: $script_path" >&2
        exit 1
    fi
    ...
    log_file="train/tinyvla/${method}/nohup_out/$(date +"%Y-%m-%d")/${SUITE_STAMP}-cl_suite.log"
...
python - <<'PY' "$MANIFEST_TSV" "$MANIFEST_JSON" "$SUITE_STAMP" "$MONITOR_INTERVAL_SECONDS" "$PLOT_INTERVAL_SECONDS" "$INHERITED_SUITE_LABEL" "$RESOURCE_CHANGE_TIME_POINTS" "$RESOURCE_CHANGE_DIRECTIONS" "$RESOURCE_CHANGE_FACTORS" "$SMOKE" "${mwe_per_method_runtime_hours:-}"
import csv
import json
import sys
...
smoke = sys.argv[10] == "1"
smoke_max_runtime_hours = float(sys.argv[11]) if sys.argv[11] else None
...
python "$ROOT_DIR/train/octo/spawn_detached.py" \
    ...
    --cwd "$ROOT_DIR" \
    -- python -u -m train.tinyvla.plot_cl_suite \
