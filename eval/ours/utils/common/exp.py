...
def save_models_dict_for_init(models_dict, exp_entry_file, uid=None):
    target_filename = os.path.basename(exp_entry_file) if uid is None else os.path.basename(exp_entry_file) + '_' + uid
    target_file_path = os.path.join(os.path.dirname(exp_entry_file), f'entry_model/{target_filename}.pt')
    # if os.path.exists(target_file_path):
    #     logger.info(f'NOTE: use cached model to ensure reproducability!!!')
