...
def load_agent():
    ...
    agent.load_state_dict(torch.load(args.ppo_pretrained_model1_path)['agent'])
...
def load_agent2():
    ...
    agent = AgentWithoutDepthInput(actor, 256 * 2, state_max, state_min, args.normalize_states).to(device)
    ...
    agent.load_state_dict(torch.load(args.ppo_pretrained_model2_path)['agent'])
...
def ppo_agent(args, device, base_runname, agent, agent_name, layer_name_of_output_features: str, layers_name_of_head: List[str],
              local_feature_dim: int, feature_selector_alpha: float):
    ...
    for n, p in agent.named_parameters():
        if args.head_learning_rate > 0 and any([n.startswith(layer_name) for layer_name in layers_name_of_head]):
            p.requires_grad = True
    ...
    if len(trainable_parameters) > 0:
        optimizer = optim.Adam(
            [{'params': trainable_parameters, 'lr': args.head_learning_rate}], eps=1e-5
        )
    else:
        optimizer = optim.Adam(
    ...
    for iteration in range(start_iter_idx, args.num_iterations + 1):
        ...
        agent.eval()
        if iteration % args.eval_freq == 1 or iteration == start_iter_idx:
        ...
        for client_id, fap in feature_aggregators_parameters.items():
            if client_id not in training_feature_aggregator_modules:
                ...
                for p in fap_list:
                    p.requires_grad = True
                optimizer.add_param_group({'params': fap_list, 'lr': args.learning_rate, 'eps': 1e-5})
...
if __name__ == "__main__":
    ...
    data_manager_proc = subprocess.Popen(
        ['python', '-m', 'uvicorn', 'ours.de_feature_fusion.data_manager:app', '--host', '0.0.0.0', '--port', '8000'],
