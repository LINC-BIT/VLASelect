...
filter_method_smoke_overrides() {
    ...
    for entry in "${raw_overrides[@]}"; do
        ...
        if [[ "$method" == "ours" ]]; then
            case "$key" in
                NUM_ENVS_OVERRIDE|NUM_EVAL_ENVS_OVERRIDE|NUM_STEPS_OVERRIDE|NUM_MINIBATCHES_OVERRIDE|UPDATE_EPOCHS_OVERRIDE|ROLLOUT_MICRO_BATCH_SIZE_OVERRIDE|EVAL_MICRO_BATCH_SIZE_OVERRIDE|UPDATE_MICRO_BATCH_SIZE_OVERRIDE|MAX_RUNTIME_HOURS_OVERRIDE)
                    continue
                    ;;
            esac
        fi
