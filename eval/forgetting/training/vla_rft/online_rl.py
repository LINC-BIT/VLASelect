...
from train.octo.vla_rft.pretrain_world_model import DynamicsWorldModel, StateNormalizer
...
class Args:
    ...
    curve_title: str = "VLA-RFT Success Curve"
...
class WorldModelRewarder:
    def __init__(self, ckpt_path: str, device: torch.device):
        if not ckpt_path:
            raise ValueError("--world-model-checkpoint is required for VLA-RFT online training")
...
def build_agent_from_checkpoint(args: Args, device: torch.device, env_kwargs: dict):
    if args.small_model_generation_strategy != "source":
        raise ValueError("VLA-RFT baseline fixes small_model_generation_strategy to 'source'")
...
def save_checkpoint(run_name, filename, agent, optimizer, iteration, extra_metrics=None, ewc_state=None):
...
def main():
    ...
    if args.exp_name is None:
        run_name = f"{args.env_id}/baselines/vla_rft/{Path(__file__).stem}/{datetime.now().strftime('%Y%m%d-%H%M%S')}"
    ...
    envs, eval_envs, env_kwargs = make_envs_for_env_id(
        args,
        current_env_id,
        env_kwargs,
        run_name,
    ...
    def maybe_switch_envs():
        ...
        if scheduled_env_index >= len(continual_env_schedule.env_ids):
            ...
            record_past_env_snapshot(
                ...
                make_env_pair=lambda env_id, env_index: make_envs_for_env_id(
                    args, env_id, env_kwargs, run_name, env_index, args.evaluate
                ),
            )
        ...
        envs, eval_envs, _ = make_envs_for_env_id(
            args,
            current_env_id,
            env_kwargs,
            run_name,
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(args.num_steps):
            ...
            with torch.no_grad():
                ...
                verified_reward, reward_info_last = rewarder.compute_reward(next_obs, action, args)
        ...
        for _ in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                task_loss = pg_loss - args.ent_coef * entropy_loss + args.vf_coef * v_loss
    ...
    if args.save_model:
        ewc_state.consolidate(agent)
        save_checkpoint(run_name, "last.pt", agent, optimizer, last_iteration, None, ewc_state=ewc_state)
