...
# TinyVLA is intentionally launched with the same HoldCubeInHand task contract and
# continual-learning hyperparameters as the VLA-Adapter verification script.
MWE=${MWE:-0}
...
ARGS=(--env-id "$ENV_ID" --output-dir "$OUTPUT_DIR" \
  ...
  --freeze-vla-backbone false --backbone-warmup-updates 0 --save-video false --action-dim 8 --state-dim 44 --env-action-dim 16 --controlled-action-indices "(0,1,2,3,4,5,6,7)" \
...
exec python -u "$SCRIPT_DIR/tinyvla_impl.py" "${ARGS[@]}"
