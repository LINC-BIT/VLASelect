...
DEFAULT_OUTPUT_DIR = "train/tinyvla/conrft/outputs"
DEFAULT_TEACHER_CHECKPOINT = (
    "ckpt/tinyvla/model_impl/outputs/ppo_open_cabinet_drawer/20260507-113650/best_policy.pt"
)
...
DEFAULT_SUMMARY_NAME = "conrft_training_summary.json"
...
class Args:
    ...
    supervised_learning_rate: float = 6e-5
    ...
    warmup_success_steps_before_supervised: int = 100
    min_success_steps_for_supervised: int = 10
    online_buffer_capacity: int = 20_000
    expert_buffer_capacity: int = 40_000
    expert_target_success_trajectories: int = 24
    expert_collect_num_envs: int = 8
    expert_collect_max_steps: int = 5_000
    expert_collect_seed: int = 0
...
def build_teacher_policy(args: Args, device: torch.device) -> reference.EdgeVLAActorCritic:
...
def build_static_student_policy(args: Args, device: torch.device) -> reference.EdgeVLAActorCritic:
    ...
    static_policy.configure_trainable_modules(train_backbone=not args.freeze_vla_backbone)
...
def build_supervised_optimizer(
    ...
    params = []
    for module in [policy.vla, policy.state_projector, policy.context_projector, policy.actor_head]:
        for parameter in module.parameters():
            if parameter.requires_grad:
                params.append(parameter)
    return optim.AdamW(params, lr=args.supervised_learning_rate, eps=1e-5, weight_decay=args.weight_decay)
...
def collect_successful_trajectories(
    ...
    buffer: ActionBinTrajectoryBuffer,
    max_steps: int,
    seed: int,
    prefix: str,
) -> Dict[str, float]:
    ...
    active_episode_buffers: List[List[Dict[str, torch.Tensor]]] = [[] for _ in range(envs.num_envs)]
    completed_episodes = 0
    total_steps = 0
    start_time = time.time()

    while buffer.num_success_trajectories < target_success_trajectories and total_steps < max_steps:
        step_rgbs = reference.extract_rgb_batch_from_obs(obs)
        step_states_np = reference.extract_cabinet_state_batch_from_obs(obs)
        step_states_cpu = torch.from_numpy(step_states_np).to(dtype=torch.float32)

        action, _, _, _, action_bins = reference.batched_get_action_and_value_no_grad(
            policy,
            step_rgbs,
            step_states_np,
        ...
        done_indices = torch.arange(envs.num_envs, device=done_mask.device)[done_mask]
        completed_episodes += int(done_mask.sum().item())
        for env_idx in done_indices.detach().cpu().tolist():
            ...
            if success_once[env_idx].item() > 0 and len(episode_samples) >= args.min_success_steps_for_supervised:
                buffer.add_episode(episode_samples)
            active_episode_buffers[env_idx] = []

    elapsed = time.time() - start_time
    stats = {
        "success_trajectories": float(buffer.num_success_trajectories),
        "success_steps": float(len(buffer)),
        "episodes_finished": float(completed_episodes),
        "collect_steps": float(total_steps),
        "elapsed_seconds": float(elapsed),
    }
    print(
        f"[{prefix}] success_trajs={buffer.num_success_trajectories} success_steps={len(buffer)} "
        f"episodes_finished={completed_episodes} collect_steps={total_steps} elapsed_s={elapsed:.1f}"
    )
    return stats


def sample_supervised_batch(
    expert_buffer: ActionBinTrajectoryBuffer,
    ...
    if len(expert_buffer) > 0 and len(online_buffer) > 0:
        online_size = min(batch_size, int(round(batch_size * online_ratio)))
        expert_size = batch_size - online_size
        if online_size == 0:
            expert_size = batch_size
        if expert_size == 0:
            online_size = batch_size
    elif len(expert_buffer) > 0:
        expert_size = batch_size
    elif len(online_buffer) > 0:
        online_size = batch_size
    else:
        raise ValueError("both expert and online buffers are empty")

    batches = []
    if expert_size > 0:
        batches.append(expert_buffer.sample_batch(expert_size, device))
    if online_size > 0:
...
def run_supervised_stage(
    ...
    if len(online_buffer) < args.warmup_success_steps_before_supervised:
        ...
        online_ratio = args.supervised_online_ratio
    ...
    for _ in range(args.supervised_updates_per_iter):
        ...
        for start, end in reference.iter_slices(total, args.update_micro_batch_size):
            ...
            policy_action_dim = int(getattr(policy, "policy_action_dim", policy.env_action_dim))
            bc_loss = (-log_prob.mean()) / policy_action_dim
    ...
    return {
        ...
        "bc_online_ratio": float(online_ratio),
...
def train(args: Args) -> None:
    ...
    expert_buffer = ActionBinTrajectoryBuffer(args.expert_buffer_capacity)
    expert_collect_stats: Dict[str, float] = {}
    if args.expert_target_success_trajectories > 0:
        print("[setup] collecting fixed teacher demonstrations for ConRFT supervision")
        teacher_policy = build_teacher_policy(args, device)
        expert_envs = make_vector_env_for_env_id(
            runtime_args,
            device,
            current_env_id,
            args.expert_collect_num_envs,
            record_metrics=True,
        )
        try:
            expert_collect_stats = collect_successful_trajectories(
                args=args,
                policy=teacher_policy,
                envs=expert_envs,
                target_success_trajectories=args.expert_target_success_trajectories,
                buffer=expert_buffer,
                max_steps=args.expert_collect_max_steps,
                seed=args.seed + args.expert_collect_seed,
                prefix="expert-collect",
            )
        finally:
            expert_envs.close()
            del teacher_policy
        ...
        print("[setup] expert_target_success_trajectories <= 0, skip fixed teacher demo collection")

    raw_policy = build_static_student_policy(args, device)
    ...
    initial_metric = {
        ...
        "bc_online_ratio": 0.0,
        "bc_updates": 0.0,
        "expert_buffer_steps": float(len(expert_buffer)),
        "expert_success_trajectories": float(expert_buffer.num_success_trajectories),
        ...
        for step in range(args.num_steps):
            ...
            if done_mask is not None:
                ...
                if "final_observation" in infos and bootstrap_mask.any():
                    ...
                    final_values[step, bootstrap_mask] = reference.batched_get_value_no_grad(
                        raw_policy,
                        final_rgbs,
                        final_states,
    ...
    save_summary(
        output_dir,
        {
            ...
            "expert_collect_stats": expert_collect_stats,
            "expert_buffer_steps": len(expert_buffer),
            "expert_success_trajectories": expert_buffer.num_success_trajectories,
