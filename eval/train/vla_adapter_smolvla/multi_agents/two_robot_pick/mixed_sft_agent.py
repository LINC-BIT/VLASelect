...
class MixedTinyVLAAdapterSmolVLASFTAgent(nn.Module):
    def __init__(
        self,
    ...
    def load_checkpoint_state_dict(self, state_dict):
        ...
        for key, value in state_dict.items():
            ...
            matched[key] = value
        if not matched:
            raise RuntimeError("No compatible parameters found for MixedTinyVLAAdapterSmolVLASFTAgent")
        ...
        print(
            "[Checkpoint] MixedTinyVLAAdapterSmolVLASFTAgent "
            f"matched={len(matched)} skipped={len(skipped)} missing={len(missing)}"
        )


def build_mixed_mappo_optimizer(args, agent: MixedTinyVLAAdapterSmolVLASFTAgent) -> torch.optim.Optimizer:
    param_groups = [
        {
