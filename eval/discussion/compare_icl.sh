...
run_vlaselect() {
    ...
    if [[ "$MWE" == "1" ]]; then
        env             CUDA_DEVICES="$CUDA_DEVICES"             EXP_NAME="$exp_name"             LAUNCH_DIRECT=1             TAIL_LOG="$TAIL_LOG"             ENV_ID_OVERRIDE=PickCubeObjectScaleUp1p2-v1             ENVS_ID_OVERRIDE="['PickCubeObjectScaleUp1p2-v1']"             ENV_CHANGE_TIME_POINTS_OVERRIDE="[1000000]"             ENABLE_RICL_INJECTION=0             NUM_ENVS_OVERRIDE=8             NUM_EVAL_ENVS_OVERRIDE=2             NUM_STEPS_OVERRIDE=16             NUM_EVAL_STEPS_OVERRIDE="$ICL_NUM_EVAL_STEPS"             NUM_MINIBATCHES_OVERRIDE=2             UPDATE_EPOCHS_OVERRIDE=1             WANDB_MODE=disabled             WANDB_SILENT=true             MWE_ACTIVE_RUNTIME_ONLY=1             MAX_TIME_OVERRIDE="$MWE_PER_METHOD_RUNTIME_MINUTES"             bash "${EVAL_ROOT}/train/octo/ours_single_agent/online_rl_ours_single_agent_cl.sh"
    else
        env             CUDA_DEVICES="$CUDA_DEVICES"             EXP_NAME="$exp_name"             LAUNCH_DIRECT=1             TAIL_LOG="$TAIL_LOG"             ENV_ID_OVERRIDE=PickCubeObjectScaleUp1p2-v1             ENVS_ID_OVERRIDE="['PickCubeObjectScaleUp1p2-v1']"             ENV_CHANGE_TIME_POINTS_OVERRIDE="[1000000]"             NUM_EVAL_STEPS_OVERRIDE="$ICL_NUM_EVAL_STEPS"             bash "${EVAL_ROOT}/train/octo/ours_single_agent/online_rl_ours_single_agent_cl.sh"
    fi
}
