...
from ours.pretrain_fbs_model.main import set_sparsity
...
DEFAULT_WORKDIR = "train/vla_adapter_new/ours/outputs"
DEFAULT_TEACHER_CHECKPOINT = (
    "ckpt/vla_adapter_new/model_impl/outputs/ppo_hold_cube_in_hand/20260430-103518/best_policy.pt"
)
DEFAULT_STUDENT_INIT = "train/vla_adapter_new/ours/pretrained_model_with_fbs.pth"
...
class Args:
    ...
    resume_from: Optional[str] = DEFAULT_STUDENT_INIT
    num_envs: int = 256
    num_eval_envs: int = 8
    num_steps: int = 50
    ...
    head_learning_rate: float = 1e-6
    state_learning_rate: float = 1e-6
    value_head_learning_rate: float = 1e-6
...
def maybe_load_policy_checkpoint(
    checkpoint_path: Optional[str],
    raw_policy: reference.HandVLAAdapterActorCritic,
...
def evaluate_policy_at_sparsities(
    ...
    try:
        for sparsity in sparsities:
            set_sparsity_fn(policy, sparsity)
        ...
        if previous_sparsity is not None:
            set_sparsity_fn(policy, previous_sparsity)
...
def collect_teacher_rollout(
    ...
    for _ in range(args.num_steps):
        ...
        step_states_np = reference.extract_hand_state_batch_from_obs(next_obs)
    ...
    rollout_summary = gather_metric_summary(reference.summarize_episode_metrics(train_episode_metrics))
    rollout_summary["reward_mean"] = distributed_mean(float(reward_tensor.mean().item()), teacher_policy.device)
    rollout_summary["teacher_return_mean"] = distributed_mean(float(reward_tensor.sum(dim=0).mean().item()), teacher_policy.device)
        ...
        for start, end in iter_slices(total, local_minibatch_size):
            ...
            if is_main_process():
                if minibatch_idx_in_epoch % 4 == 0:
                    ...
                    cur_sparsity = np.random.uniform(0.0, 0.8)
                else:
                    cur_sparsity = 0.8
            else:
                cur_sparsity = None
            ...
            for micro_start, micro_end in iter_slices(minibatch_total, args.update_micro_batch_size):
                ...
                # `log_prob` sums across action dimensions; dividing by action_dim
                # matches the usual mean cross-entropy-per-token scale.
...
def plot_bc_metrics_history(
    ...
    def minibatch_loss_series() -> Tuple[List[int], List[float]]:
        xs: List[int] = []
        ys: List[float] = []
        for point in minibatch_loss_history:
            xs.append(int(point["optimizer_step"]))
            ys.append(float(point["loss"]))
        return xs, ys
    ...
    subplot_specs = [
        ...
        (
            "Eval Success Once",
            "Success Rate",
            [f"eval_sparsity_{sparsity_tag(s)}_success_once" for s in EVAL_SPARSITIES],
        ),
        (
            "Eval Success At End",
            "Success Rate",
            [f"eval_sparsity_{sparsity_tag(s)}_success_at_end" for s in EVAL_SPARSITIES],
        ),
    ]
    plotted_any = False
    ax = plt.subplot(2, 2, 1)
    loss_xs, loss_ys = minibatch_loss_series()
    if loss_xs:
        plotted_any = True
    ...
    for subplot_idx, (title, ylabel, keys) in enumerate(subplot_specs, start=2):
    ...
    if loss_xs:
        plt.plot(loss_xs, loss_ys, marker="o", linewidth=1.5, markersize=3)
        plt.xlabel("Optimizer Step")
        plt.ylabel("BC Loss")
        plt.title("BC Loss By Optimizer Step")
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig(plots_dir / "bc_loss_by_optimizer_step.png", dpi=200)
    plt.close()

    plt.figure(figsize=(9, 6))
    plotted_success_once = False
    for sparsity in EVAL_SPARSITIES:
        key = f"eval_sparsity_{sparsity_tag(sparsity)}_success_once"
        xs, ys = series(key)
        if not xs:
            continue
        plotted_success_once = True
        plt.plot(xs, ys, marker="o", linewidth=2, label=f"sparsity={sparsity:.1f}")
    if plotted_success_once:
        plt.xlabel("Update")
        plt.ylabel("Success Once")
        plt.title("Eval Success Once By Sparsity")
        plt.grid(True, alpha=0.3)
        plt.legend()
        plt.tight_layout()
        plt.savefig(plots_dir / "eval_success_once_by_sparsity.png", dpi=200)
    plt.close()
...
def train(args: Args) -> None:
    ...
    if is_main_process():
        ...
        print(f"[setup] teacher_checkpoint={args.teacher_checkpoint}")
        print(f"[setup] student_resume_from={args.resume_from}")
    ...
    maybe_load_policy_checkpoint(args.teacher_checkpoint, teacher_policy)
    freeze_module(teacher_policy)
    teacher_policy.eval_micro_batch_size = args.rollout_micro_batch_size

    student_policy = reference.HandVLAAdapterActorCritic(
        Path(args.model_dir),
        device=device,
        state_dim=args.state_dim,
        action_dim=args.action_dim,
    ...
    start_update, global_step, optimizer_step, best_success_once = maybe_load_policy_checkpoint(
        args.resume_from,
        student_policy,
        optimizer,
    )
    ...
    for update in range(start_update, num_updates + 1):
        ...
        if is_main_process() and eval_envs is not None and (update % args.eval_every_updates == 0 or update == num_updates):
            ...
            eval_metrics = evaluate_policy_at_sparsities(
                student_policy,
                eval_envs,
                args.eval_episodes,
                set_sparsity_fn=set_sparsity,
            )
        ...
        if is_main_process():
            ...
            print(
                ...
                f"optimizer_step={optimizer_step} bc_loss={metric['bc_loss']:.6f} reward={metric['reward_mean']:.4f} "
    ...
    if is_main_process():
        if eval_envs is not None:
            final_eval_metrics_flat = evaluate_policy_at_sparsities(
                student_policy,
                eval_envs,
                args.eval_episodes,
                set_sparsity_fn=set_sparsity,
