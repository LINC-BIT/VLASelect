...
class HandVLAAdapterActorCritic(nn.Module):
    ...
    def _build_value_input(
        ...
        # Keep critic gradients from moving shared policy features during PPO.
        return torch.cat(
...
def get_optimizer_lrs(optimizer: optim.Optimizer) -> List[float]:
    return [float(param_group["lr"]) for param_group in optimizer.param_groups]


def set_optimizer_lrs(optimizer: optim.Optimizer, lrs: List[float]) -> None:
    if len(lrs) != len(optimizer.param_groups):
        raise ValueError(f"Expected {len(optimizer.param_groups)} learning rates, got {len(lrs)}")
...
class Args:
    ...
    kl_backtrack_attempts: int = 10
    kl_backtrack_factor: float = 0.5
    kl_probe_batch_size: int = 128
    kl_initial_step_scale: float = 5e-5
    kl_step_scale_growth: float = 2.0
...
def select_kl_probe_inds(inds: np.ndarray, probe_batch_size: int) -> np.ndarray:
    if probe_batch_size <= 0 or len(inds) <= probe_batch_size:
        return inds
    positions = np.linspace(0, len(inds) - 1, num=probe_batch_size, dtype=np.int64)
    return inds[positions]
...
def ppo_update_with_micro_batches(
    ...
    if skipped_on_kl:
        ...
        stats["post_step_probe_kl"] = 0.0
        stats["post_step_full_kl"] = 0.0
        stats["kl_step_scale"] = 0.0
        stats["kl_backtrack_attempts"] = 0.0
        stats["grad_norm"] = 0.0
        return dict(stats)

    raw_policy = unwrap_policy(policy)
    parameter_snapshot = snapshot_trainable_parameters(raw_policy)
    average_trainable_gradients(raw_policy)
    grad_norm = nn.utils.clip_grad_norm_(raw_policy.parameters(), args.max_grad_norm)
    stats["grad_norm"] = float(grad_norm.detach().item() if isinstance(grad_norm, torch.Tensor) else grad_norm)
    full_probe_inds = minibatch_inds if post_step_probe_inds is None else post_step_probe_inds
    original_lrs = get_optimizer_lrs(optimizer)
    max_post_step_full_kl = None
    if args.target_kl is not None:
        max_post_step_full_kl = args.target_kl
    attempts = max(1, args.kl_backtrack_attempts)
    accepted = False
    post_step_probe_kl = 0.0
    post_step_full_kl = 0.0
    accepted_scale = 0.0
    accepted_full_kl = 0.0
    attempts_used = 0

    step_scale = float(getattr(args, "_current_kl_step_scale", args.kl_initial_step_scale))
    low_scale = 0.0
    ...
    for attempt in range(attempts):
        ...
        set_optimizer_lrs(optimizer, [lr * step_scale for lr in original_lrs])
        optimizer.step()

        post_step_full_kl = distributed_max(
            compute_teacher_forcing_approx_kl(
                ...
                inds=full_probe_inds,
            ),
            raw_policy.device,
        )
        post_step_probe_kl = post_step_full_kl
        if max_post_step_full_kl is None or post_step_full_kl <= max_post_step_full_kl:
            accepted = True
            accepted_scale = step_scale
            accepted_full_kl = post_step_full_kl
            low_scale = step_scale
            if is_main_process():
                print(
                    f"[kl-backtrack] attempt={attempts_used}/{attempts} scale={step_scale:.8g} "
                    f"full_kl={post_step_full_kl:.5f} status=accept_trial",
                    flush=True,
                )
            if high_scale is None:
                step_scale = min(1.0, max(step_scale * args.kl_step_scale_growth, step_scale + 1e-12))
            else:
                next_scale = 0.5 * (low_scale + high_scale)
                if next_scale <= low_scale * (1.0 + 1e-3):
                    break
                step_scale = next_scale
            continue

        high_scale = step_scale
        if is_main_process():
            print(
                f"[kl-backtrack] attempt={attempts_used}/{attempts} scale={step_scale:.8g} "
                f"full_kl={post_step_full_kl:.5f} status=full_reject",
                flush=True,
            )
        if low_scale > 0.0:
            step_scale = 0.5 * (low_scale + high_scale)
        else:
            step_scale *= max(args.kl_backtrack_factor, 1e-12)

    stats["post_step_probe_kl"] = post_step_probe_kl
    stats["post_step_full_kl"] = post_step_full_kl
    stats["kl_step_scale"] = accepted_scale
    stats["kl_backtrack_attempts"] = float(attempts_used)

    if accepted:
        restore_trainable_parameters(raw_policy, parameter_snapshot)
        reset_optimizer_state_for_trainable_params(optimizer, raw_policy)
        set_optimizer_lrs(optimizer, [lr * accepted_scale for lr in original_lrs])
        optimizer.step()
        post_step_probe_kl = accepted_full_kl
        post_step_full_kl = accepted_full_kl
        stats["post_step_probe_kl"] = accepted_full_kl
        stats["post_step_full_kl"] = accepted_full_kl
        next_scale = 0.5 * (accepted_scale + high_scale) if high_scale is not None else accepted_scale
        setattr(args, "_current_kl_step_scale", min(1.0, max(next_scale, 1e-12)))
        set_optimizer_lrs(optimizer, original_lrs)
        optimizer.zero_grad(set_to_none=True)
    ...
    restore_trainable_parameters(raw_policy, parameter_snapshot)
    reset_optimizer_state_for_trainable_params(optimizer, raw_policy)
    setattr(args, "_current_kl_step_scale", max(step_scale, 1e-12))
    set_optimizer_lrs(optimizer, original_lrs)
    optimizer.zero_grad(set_to_none=True)
    stats["skipped_on_kl"] = 1.0
...
def train(args: Args) -> None:
    ...
    for update in range(start_update, num_updates + 1):
        ...
        _, b_logprobs, _, b_values, _ = batched_recompute_action_and_value_detached(
            policy,
            b_rgbs,
            b_states,
        ...
        b_logprobs = b_logprobs.detach()
        b_values = b_values.detach()
        values_buf = b_values.view(args.num_steps, local_num_envs)
        logprobs_buf = b_logprobs.view(args.num_steps, local_num_envs)
        ...
        post_step_probe_kl_value = 0.0
        post_step_full_kl_value = 0.0
        kl_step_scale_value = 0.0
        kl_backtrack_attempts_value = 0.0
        grad_norm_value = 0.0
        ...
        for epoch in range(args.update_epochs):
            ...
            post_step_probe_kl_value = distributed_mean(
                float(np.mean(epoch_stats["post_step_probe_kl"])) if epoch_stats["post_step_probe_kl"] else 0.0,
                device,
            )
            post_step_full_kl_value = distributed_mean(
                float(np.mean(epoch_stats["post_step_full_kl"])) if epoch_stats["post_step_full_kl"] else 0.0,
                device,
            )
            kl_step_scale_value = distributed_mean(
                float(np.mean(epoch_stats["kl_step_scale"])) if epoch_stats["kl_step_scale"] else 0.0,
                device,
            )
            kl_backtrack_attempts_value = distributed_mean(
                float(np.mean(epoch_stats["kl_backtrack_attempts"])) if epoch_stats["kl_backtrack_attempts"] else 0.0,
                device,
            )
            grad_norm_value = distributed_mean(
                float(np.mean(epoch_stats["grad_norm"])) if epoch_stats["grad_norm"] else 0.0,
                device,
            )
        ...
        metric = {
            ...
            "post_step_probe_kl": post_step_probe_kl_value,
            "post_step_full_kl": post_step_full_kl_value,
            "kl_step_scale": kl_step_scale_value,
            "kl_backtrack_attempts": kl_backtrack_attempts_value,
        ...
        if is_main_process():
            ...
            print(
                ...
                f"approx_kl={metric['approx_kl']:.5f} post_step_probe_kl={metric['post_step_probe_kl']:.5f} "
                f"post_step_full_kl={metric['post_step_full_kl']:.5f} "
                f"kl_scale={metric['kl_step_scale']:.8g} kl_attempts={metric['kl_backtrack_attempts']:.1f} "
