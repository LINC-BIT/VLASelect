...
import env as hold_cube_env  # noqa: F401
...
DEFAULT_WORKDIR = "train/vla_adapter_new/model_impl/outputs/ppo_hold_cube_in_hand"
...
class HandVLAAdapterActorCritic(nn.Module):
    def __init__(self, model_dir: Path, device: torch.device, state_dim: int = 105, action_dim: int = 16, architecture_config: Optional[Dict[str, Any]] = None):
        super().__init__()
        ...
        if not model_dir.is_dir():
            raise FileNotFoundError(f"VLA-Adapter model directory does not exist: {model_dir}")
