...
def set_action(action):
    # action : [1, 8]
    # for finger i: (i: 1~4)
    # i * 2 left servo (angle 2)
    # i * 2 + 1 right servo (angle 1)
    # for action index j:
    # j // 2 + 1 => finger id
    # j 双数为 left servo : rad 2 ~ 2
    # 单数为 right servo : rad -2 ~ 2
    ...
    action = action / torch.pi * 180
    for i in range(4):
        left = action[i * 2]
        right = action[i * 2 + 1]
        if i == 0:
            Move_Index(right, left, MaxSpeed)
        elif i == 1:
            Move_Middle(right, left, MaxSpeed)
        elif i == 2:
            Move_Ring(right, left, MaxSpeed)
        elif i == 3:
            Move_Thumb(right, left, MaxSpeed)

def read_qpos():
    qpos = []
    for i in range(1, 5):
        right = c.read_goal_position(i * 2 - 1)
        time.sleep(0.0002)
        left = c.read_goal_position(i * 2)
        time.sleep(0.0002)
        qpos += left
        qpos += right
    return torch.tensor(qpos).unsqueeze(0)
...
if __name__ == '__main__':
    # main()
    set_action(torch.tensor([0, 0, 0, 0, 0, 0, 0, 0]))
