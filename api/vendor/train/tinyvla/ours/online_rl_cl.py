...
import workloads.mobile_arm as mobile_arm_workload  # noqa: F401
...
DEFAULT_WORKDIR = "train/tinyvla/ours/outputs/online_rl_cl"
DEFAULT_VERIFY_SUMMARY_NAME = "edgevla_ours_cl_summary.json"
DEFAULT_FBS_CHECKPOINT = "ckpt/tinyvla/ours/outputs/bc_open_cabinet_drawer_fbs/20260508-032529/best_policy.pt"
...
class Args:
    ...
    envs_id: str = "['OpenCabinetDrawerEasyLevel0-v1', 'OpenCabinetDrawerEasyLevel0-v1']"
    ...
    early_stop_zero_success_minutes: float = 45000.0
...
def collect_sample_for_small_model_scaling(
    ...
    if args.small_model_scaling_strategy in {"target-batch", "target-single"}:
        obs, _ = eval_envs.reset()
        rgbs = reference.extract_rgb_batch_from_obs(obs)
        states = reference.extract_cabinet_state_batch_from_obs(obs)
        ...
        if args.small_model_scaling_strategy == "target-single":
            return {
                "rgbs": rgbs[:1].clone(),
    ...
    if args.small_model_scaling_strategy == "target-single-traj":
    ...
    raise NotImplementedError(f"Unknown small_model_scaling_strategy: {args.small_model_scaling_strategy}")
...
def regenerate_small_model_in_place(
    ...
    sample = collect_sample_for_small_model_scaling(args, large_agent=large_agent, small_agent=small_agent, eval_envs=eval_envs, device=device)
...
def train(args: Args) -> None:
    ...
    initial_sample = collect_sample_for_small_model_scaling(
        args,
