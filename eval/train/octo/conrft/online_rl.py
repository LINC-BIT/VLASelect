...
class Args:
    ...
    supervised_updates_per_iter: int = 16
...
def main():
    ...
    if args.exp_name is None:
        ...
        run_name = f"{args.env_id}/baselines/conrft/{args.exp_name}/{datetime.now().strftime('%Y%m%d-%H%M%S')}"
    ...
    print("ConRFT baseline uses PPO online RL + symmetric BC updates from demos and successful online trajectories.")
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if switched_env:
            print(f"Switched to env[{current_env_index}]={current_env_id} before iteration {iteration}")
        ...
        if logger is not None and elapsed_minutes is not None:
            ...
            logger.add_scalar("continual/current_env_index", current_env_index, global_step)
        if switched_env:
            print(f"Switched to env[{current_env_index}]={current_env_id} after iteration {iteration}")
        if should_stop_for_schedule:
            print(
