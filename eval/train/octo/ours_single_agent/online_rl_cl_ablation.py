...
class Args:
    ...
    small_model_feedback_strategy: Optional[str] = None
...
def ppo_agent(args: Args, device, base_runname, agent, agent_name, layer_name_of_output_features: str, layers_name_of_head: List[str],
    ...
    freeze_hook_handles = []
    if args.small_model_feedback_strategy not in {None, 'random'}:
        raise ValueError(
            f'Unknown small_model_feedback_strategy: {args.small_model_feedback_strategy}'
        )
    ...
    for iteration in range(start_iter_idx, args.num_iterations + 1):
        ...
        if os.environ.get("MWE", "0") == "1":
            if args.small_model_feedback_strategy == 'random' and args.mwe_regeneration_count > 0:
                feedback_due = mwe_operation_due(
                    args.mwe_regeneration_count,
                    mwe_feedback_count,
                    runtime_tracker.current_minutes(),
                    args.max_time,
                )
            elif args.mwe_knowledge_accumulation_count > 0:
                feedback_due = mwe_operation_due(
        ...
        if regeneration_due:
            ...
            current_small_model_pruning_info, forward_seconds, enhancer_seconds = regenerate_small_model_in_place(
                ...
                env_kwargs=env_kwargs,
                device=device,
                force_full_replacement=args.small_model_feedback_strategy == 'random',
            )
            module_breakdown["large_model_forward_seconds"] += forward_seconds
            module_breakdown["small_model_generation_seconds"] += enhancer_seconds
            update_combined_search_enhancement_seconds(module_breakdown)
