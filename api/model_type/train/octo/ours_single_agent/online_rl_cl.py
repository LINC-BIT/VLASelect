...
MODEL_TYPE = os.environ.get("MODEL_TYPE", "cnn").strip().lower()


def is_mlp_model() -> bool:
...
class Args:
    ...
    pre_generated_model: Optional[str] = None
    """load a fixed, already-generated small model and skip VLASelect setup"""
    generate_only: bool = False
    generated_model_output: Optional[str] = None
...
def build_initial_trainable_small_model(args, large_agent, eval_envs, env_kwargs, device):
    ...
    if is_mlp_model():
        from mlp_model import generate_small_mlp_with_verify

        small_agent, pruning_info = generate_small_mlp_with_verify(
            large_agent,
            args.max_sparsity,
...
def feedback_small_model_to_large_model(
    ...
    if is_mlp_model():
        from mlp_model import feedback_small_mlp_to_large_model

        feedback_small_mlp_to_large_model(
            large_agent,
            small_agent,
...
def regenerate_small_model_in_place(
    ...
    if is_mlp_model():
        ...
        regenerated_small_agent, new_pruning_info = generate_small_mlp_with_verify(
            large_agent,
            args.max_sparsity,
            sample_for_gen_small_model,
            return_pruning_info=True,
        ...
        return new_pruning_info, forward_seconds, time.perf_counter() - enhancer_start_time
...
def load_agent():
    if is_mlp_model():
        ...
        agent = MLPAgent(
            state_dim=42,
            action_dim=4,
            actor_logstd=args.actor_logstd,
            state_max=state_max,
            state_min=state_min,
            # The official ManiSkill PPO MLP is trained directly on raw state observations.
        ...
        if checkpoint_path.exists():
            try:
                ...
                incompatible = agent.load_state_dict(state, strict=False)
                print(
                    f"MLP checkpoint load: missing={len(incompatible.missing_keys)} "
                    f"unexpected={len(incompatible.unexpected_keys)}"
                )
                loaded_sparsity = float(payload.get("fbs_sparsity", args.max_sparsity)) if isinstance(payload, dict) else args.max_sparsity
                set_trainable_small_model_sparsity(agent, loaded_sparsity)
                print(f"MLP FBS sparsity restored: {loaded_sparsity:.4f}")
            except Exception as exc:
                print(f"MLP checkpoint incompatible; using random initialization: {exc}")
        else:
            print(f"checkpoint not found at {checkpoint_path}; using random MLP initialization")
        return agent
...
def _strip_fbs_wrappers(model: nn.Module) -> nn.Module:
    """Return a standalone ordinary model by removing FBS wrappers."""
    ...
    for parent_name, parent in model.named_modules():
        for child_name, child in list(parent.named_children()):
            if isinstance(child, Linear2DWithFBS):
                replacements.append((parent, child_name, deepcopy(child.raw_linear)))
            elif isinstance(child, Conv2dWithFBS):
                replacements.append((parent, child_name, nn.Sequential(deepcopy(child.raw_conv2d), deepcopy(child.bn))))
    for parent, child_name, replacement in replacements:
        parent._modules[child_name] = replacement
    return model


def load_pre_generated_model(path: str, device: torch.device) -> nn.Module:
    # Older generated CNN artifacts were saved while this file ran as
    # ``__main__``.  Register the current Agent class under that module name
    # before unpickling so those fixed, extensionless artifacts remain usable
    # when the trainer is imported as a library as well.
    main_module = sys.modules.get("__main__")
    if main_module is not None and not hasattr(main_module, "Agent"):
        setattr(main_module, "Agent", Agent)
    checkpoint = torch.load(Path(path), map_location=device)
    model = checkpoint.get("model", checkpoint) if isinstance(checkpoint, dict) else checkpoint
    if not isinstance(model, nn.Module):
        raise TypeError(f"pre-generated model must contain a torch.nn.Module: {path}")
...
def ppo_agent(args: Args, device, base_runname, agent, agent_name, layer_name_of_output_features: str, layers_name_of_head: List[str],
    ...
    if args.generate_only:
        print(f"Generating standalone small model from PickCube-v1 for {args.generated_model_output}")
        sample, _ = collect_sample_for_small_model_generation(
            args=args,
            large_agent=agent,
            small_agent=None,
            eval_envs=eval_envs,
        ...
        if is_mlp_model():
            from mlp_model import generate_small_mlp_with_verify
            generated, _ = generate_small_mlp_with_verify(
                agent, args.max_sparsity, sample, return_pruning_info=True
            )
        else:
            from ours.pretrain_fbs_model.main import generate_small_cnn_with_verify
            generated, _ = generate_small_cnn_with_verify(
                agent, args.max_sparsity, sample,
                lambda model, item: model(item), return_pruning_info=True
            )
        generated = _strip_fbs_wrappers(generated).cpu()
        output_path = Path(args.generated_model_output)
        output_path.parent.mkdir(parents=True, exist_ok=True)
        torch.save({"model": generated, "source_env": "PickCube-v1", "ordinary": True}, output_path)
        print(f"Saved standalone ordinary small model to {output_path}")
    ...
    last_train_metrics = {}
    if args.pre_generated_model:
        feedback_schedule = regeneration_schedule = 'once'
    elif args.small_model_training_variant == 'frozen':
...
def apply_mwe_overrides(args: Args) -> Args:
    if os.environ.get("MWE", "0") == "1":
        ...
        mwe_runtime_minutes = float(os.environ.get("MWE_MAX_RUNTIME_MINUTES", "3.0"))
...
if __name__ == "__main__":
    ...
    # ``run.sh`` intentionally reuses stable experiment names (for example
    # ``results/cnn/compare-1``).  Refresh the code snapshot in place so a
    # rerun does not fail during setup just because the previous snapshot
    # already exists.
    shutil.copytree(
        'ours',
        f"ckpt/{run_name}/code/ours",
        dirs_exist_ok=True,
        ignore=shutil.ignore_patterns('__pycache__', 'logs', 'videos', 'utils'),
    )
    ...
    initial_agent = (
        load_pre_generated_model(args.pre_generated_model, device)
        if args.pre_generated_model
        else load_agent()
    )
    ppo_agent(
