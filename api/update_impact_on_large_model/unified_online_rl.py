...
def train(
    ...
    def collect_policy_metric(measure_envs: Any, policy: nn.Module) -> Dict[str, float]:
    ...
    def policy_accuracy(metrics: Dict[str, float]) -> float:
        value = metrics.get("train_success_once")
        if value is None:
    ...
    def collect_initial_training_metric() -> Dict[str, Any]:
        ...
        metric.update(collect_policy_metric(envs, small_agent))
        return metric
    ...
    for update in range(start_update, num_updates + 1):
        ...
        should_measure_feedback = update > start_update and (update - start_update) % 2 == 0
        if should_measure_feedback:
    ...
    plot_metrics_history(output_dir, metrics_history)
    plot_success_time_curve(output_dir, metrics_history)
    large_model_improvements = [
        record["large_model_acc_after_feedback"] - record["large_model_acc_before_feedback"]
        for record in impact_records
    ]
    save_json(
        output_dir / "impact_on_large_model.json",
        {
            "large_model_average_acc_abs_improvement_by_feedback": (
                float(sum(large_model_improvements) / len(large_model_improvements))
                if large_model_improvements
                else 0.0
            ),
