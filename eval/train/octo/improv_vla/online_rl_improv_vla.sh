...
DATE=$(date +"%Y-%m-%d")
TIME=$(date +"%H-%M-%S")
LOG_DIR="train/octo/improv_vla/nohup_out/$DATE"
...
SUPERVISED_UPDATES_PER_ITER=${SUPERVISED_UPDATES_PER_ITER_OVERRIDE:-32}
SUPERVISED_BATCH_SIZE=${SUPERVISED_BATCH_SIZE_OVERRIDE:-64}
...
RANDOM_INPUT_ROOT=${RANDOM_INPUT_ROOT_OVERRIDE:-train/octo/improv_vla/smoke_inputs}
...
EXPERT_DEMO_LOG_PREFIX=${EXPERT_DEMO_LOG_PREFIX:-octo-improv-vla-teacher-demo}
...
PYTHON_CMD=(
    python -u -m train.octo.improv_vla.online_rl
    --env-id "$ENV_ID"
    --envs-id "$ENVS_ID"
