...
DATE=$(date +"%Y-%m-%d")
TIME=$(date +"%H-%M-%S")
LOG_DIR="train/octo/convertnet/nohup_out/$DATE"
...
PYTHON_CMD=(
    ...
    --tag convertnet-targetsingletraj-feedback_per_rollout-regen_per_rollout-feedback0.5-arch_update1.0-reset_optimizer
    --max_time 301
    "${EXTRA_ARGS[@]}"
)
