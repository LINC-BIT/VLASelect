...
def resolve_small_model_interface(args):
    ...
    if args.knowledge_exchange_granularity:
        try:
            ...
            raise ValueError(f"unknown knowledge-exchange granularity {args.knowledge_exchange_granularity!r}; supported: {supported}") from exc
    return SmallModelScalingInterface()


def main() -> None:
    args = parse_args()
    run_training(make_tinyvla(), args, resolve_small_model_interface(args))
