...
def build_obs(image, qpos, qvel, tcp_pose, tcp_to_goal_pos, goal_pos, device, size=128):
    ...
    state_dict = dict(
        ...
        goal_pos=goal_pos,
        tcp_pose=tcp_pose,
        tcp_to_goal_pos=tcp_to_goal_pos
    )
...
# ==============================
class Runner:
    def __init__(self, urdf_path, ee_link, model_path, interface, goal_pos, freq=20, dof=6, device='cuda'):
        # TODO:添加接口
        # self.robot = RobotInterface()
        self.robot = interface
        self.kin = Kinematics(urdf_path, ee_link)
        self.policy = Agent(state_dim=25, action_dim=6, use_depth=False)
    ...
    def run(self, step=2):
        ...
        for i in range(step):
            ...
            # 2. Process state
            qpos_rad = self.robot.get_joint_angles()
            ...
            obs = build_obs(img, qpos_rad, qvel, tcp_pose, tcp_to_goal_pos, goal_pos, device=self.device)
...
class RobotInterface:
    def __init__(self, com, o_pose, freq=20):
        self.arm = Arm_Device(com)
        time.sleep(.1)
        ...
        self.low = np.array([-.2, -.2, -.2, -.2, -.2, -1.54])  # 这里的范围需要根据实际情况调整
        self.high = np.array([.2, .2, .2, .2, .2, 0])
        self.deg_low = np.array([0, 0, 0, 0, 0, 0])
        self.deg_high = np.array([180, 180, 180, 180, 270, 135])
        self.dt = 1 / freq * 1e3
        self.o_pose = np.array(o_pose)

        start_qpos:np.ndarray = rad2deg(np.array([0.0, 0.365, -np.pi / 2, -np.pi / 2, 0, -np.pi / 4])) + 90
        start_qpos[5] *= 2
        start_qpos = start_qpos.astype(np.uint8).tolist()

        self.arm.Arm_serial_servo_write6(*start_qpos, 1000)
        time.sleep(1)
    ...
    def get_joint_angles(self):
        # TODO: replace with hardware API
        qpos = np.zeros(6, dtype=np.float32)
        for i in range(6):
            tmp = None
            while tmp is None:
                tmp = self.arm.Arm_serial_servo_read(i + 1)
                time.sleep(1e-2)
            qpos[i] = tmp
        qpos[5] /= 2
        qpos -= 90
        qpos = deg2rad(qpos)
        self.prev_arm_action = qpos[:5].copy()
        return qpos[None, :]  # degrees
    ...
    def step(self, action):
        ...
        action[:5] += self.prev_arm_action
        self.prev_arm_action = action[:5].copy()

        action = rad2deg(action) + 90
        action[5] *= 2
        np.clip(action, self.deg_low, self.deg_high, out=action)

        self.arm.Arm_serial_servo_write6(int(action[0]), int(action[1]), int(action[2]), int(action[3]), int(action[4]), int(action[5]), int(self.dt))
