...
LOG_FILE="$LOG_DIR/$TIME-pretrain-with-fbs.log"
...
PYTHON_CMD=(
    torchrun --standalone --nproc_per_node="$NPROC_PER_NODE" "$SCRIPT_DIR/pretrain_with_fbs.py"
    --mode train
    --seed 1
    --env-id "$ENV_ID"
    ...
    --state-learning-rate 1e-6
    --value-head-learning-rate 1e-6
    ...
    --vf-coef 0.5
    --max-grad-norm 0.5
    --target-kl 100000000000000
    --minibatch-target-kl-factor 1000000000
    --eval-episodes 50
