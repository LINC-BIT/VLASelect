...
if __name__ == "__main__":
    ...
    for ds_item in tqdm(ds, total=len(ds), desc='loading samples...'):
        for k, v in ds_item.items():
            ds_dict[k] += [v.to(device)]
    for k, v in ds_dict.items():
        ds_dict[k] = torch.stack(v, dim=0)

    state_max, state_min = ds_dict['state'].max(dim=0), ds_dict['state'].min(dim=0)
    ...
    actor = Actor(ds[0]['state'].size(0), envs.single_action_space.shape[0], camera_count).to(
        device=device
    )
    ...
    pbar = tqdm(range(start_iter_idx, args.total_iters), total=args.total_iters)
