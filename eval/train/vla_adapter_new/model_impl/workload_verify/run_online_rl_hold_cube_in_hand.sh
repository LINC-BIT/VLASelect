...
WORKLOAD_VERIFY_DIR="train/vla_adapter_new/model_impl/workload_verify"
LOG_DIR="$WORKLOAD_VERIFY_DIR/nohup_out/$DATE"
LOG_FILE="$LOG_DIR/$TIME-hold-cube-in-hand.log"
OUTPUT_DIR_BASE="$WORKLOAD_VERIFY_DIR/outputs"
...
MAX_RUNTIME_HOURS=${MAX_RUNTIME_HOURS_OVERRIDE:-1.5}
EARLY_STOP_ZERO_SUCCESS_MINUTES=${EARLY_STOP_ZERO_SUCCESS_MINUTES_OVERRIDE:-45}
RUN_NAME=${RUN_NAME_OVERRIDE:-}
RESUME_FROM=${RESUME_FROM_OVERRIDE:-ckpt/vla_adapter_new/model_impl/outputs/ppo_hold_cube_in_hand/20260430-103518/best_policy.pt}
SAVE_VIDEO=${SAVE_VIDEO_OVERRIDE:-false}

PYTHON_CMD=(
    torchrun --standalone --nproc_per_node="$NPROC_PER_NODE" "$SCRIPT_DIR/online_rl_hold_cube_in_hand.py"
    --mode train
    --seed 1
    --env-id "$ENV_ID"
