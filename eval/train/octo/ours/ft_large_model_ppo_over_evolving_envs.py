...
from train.octo.evolving_envs_setting import get_num_evolving_envs, read_evolving_envs_setting
from envs.pick_obj_random import PickObjectEnv
...
class Args:
    ...
    evolving_envs_setting_fp: str = ''

    gen_small_model_strategy: str = 'source'
...
class Agent(nn.Module):
    def __init__(self, envs, feature_net, latent_size, state_max=None, state_min=None):
        super().__init__()
        ...
        self.actor_logstd = nn.Parameter(torch.ones(1, np.prod(envs.unwrapped.single_action_space.shape)) * args.actor_logstd)
...
class FlattenRGBDObservationWrapper2(gym.ObservationWrapper):
    ...
    def observation(self, observation: dict):
        ...
        # print(agent, extra)
        state_parts = []
        agent_keys = ['qpos', 'qvel']
        extra_keys = ['is_grasped', 'tcp_pose', 'goal_pos', 'obj_pose', 'tcp_to_obj_pos', 'obj_to_goal_pos']
        for k in agent_keys:
            val = agent[k]
            # print(k, val.size())
...
if __name__ == "__main__":
    ...
    # assert args.env_id == 'PickObjectRandom-v1'
    ...
    # print(f'env inited in CPU for saving memory on edge devices!')
    ...
    if args.env_id == 'PickObjectRandom-v1':
        with open(args.evolving_envs_setting_fp, 'r') as f:
            data = json.load(f)
        # for k, v in read_evolving_envs_setting(args.evolving_envs_setting_fp, args.target_evolving_env_idx).items():
    ...
    if args.capture_video:
        eval_output_dir = f"ckpt/{run_name}/videos"
        if args.evaluate:
            eval_output_dir = f"{os.path.dirname(args.checkpoint)}/test_videos"
        print(f"Saving eval videos to {eval_output_dir}")
        if args.save_train_video_freq is not None:
            save_video_trigger = lambda x : (x // args.num_steps) % args.save_train_video_freq == 0
            envs = RecordEpisode(envs, output_dir=f"ckpt/{run_name}/train_videos", save_trajectory=False, save_video_trigger=save_video_trigger, max_steps_per_video=args.num_steps, video_fps=30)
        eval_envs = RecordEpisode(eval_envs, output_dir=eval_output_dir, save_trajectory=args.evaluate, trajectory_name="trajectory", max_steps_per_video=args.num_eval_steps, video_fps=30)
    envs = ManiSkillVectorEnv(envs, args.num_envs, ignore_terminations=not args.partial_reset, record_metrics=True)
    eval_envs = ManiSkillVectorEnv(eval_envs, args.num_eval_envs, ignore_terminations=not args.eval_partial_reset, record_metrics=True)
    assert isinstance(envs.single_action_space, gym.spaces.Box), "only continuous action space is supported"

    print('envs init finished!')
    time.sleep(5)
    ...
    actor = Actor(42, envs.single_action_space.shape[0], 1, False).to(
        device=device
    )
    # add FBS
    ...
    agent = Agent(envs, actor, 256 * 3, state_max, state_min).to(device)
    ...
    if args.continue_train_from is not None:
        ...
        if 'agent' in ckpt:
            print('load pretrained agent to continue training')
            print(agent.load_state_dict(ckpt['agent'], strict=False))
            optimizer = optim.Adam(agent.parameters(), lr=args.learning_rate, eps=1e-5)
        ...
        #     global_step = start_iter_idx * args.num_envs * args.num_steps


    print(f'generate small model (by {args.gen_small_model_strategy}) for fine-tuning test')
    from ours.pretrain_fbs_model.main import generate_small_cnn_with_verify
    # eval_obs, _ = eval_envs.reset()
    # rgb, depth, state = torch.rand((1, 3, 128, 128)), torch.rand((1, 1, 128, 128)), torch.rand((1, 42))
    # sample_from_eval_obs = {
    #     'rgb': eval_obs['rgb'].to(device)[0: 1],
    #     'depth': eval_obs['depth'].to(device)[0: 1],
    #     'state': eval_obs['state'].to(device)[0: 1]
    # }


    # --------- use sample from source env to generate small model ----------
    if args.gen_small_model_strategy == 'source':
        env_kwargs_for_source = env_kwargs
        # with open(args.evolving_envs_setting_fp, 'r') as f:
        #     data = json.load(f)
        # env_kwargs_for_source = {k: v for k, v in env_kwargs.items() if k not in data}
        source_eval_envs = gym.make('PickCube-v1', num_envs=args.num_eval_envs, reconfiguration_freq=args.eval_reconfiguration_freq, **env_kwargs_for_source)
        source_eval_envs = FlattenRGBDObservationWrapper2(source_eval_envs, rgb=True, depth=True, state=args.include_state)
        if isinstance(source_eval_envs.action_space, gym.spaces.Dict):
            source_eval_envs = FlattenActionSpaceWrapper(source_eval_envs)
        source_eval_envs = ManiSkillVectorEnv(source_eval_envs, args.num_eval_envs, ignore_terminations=not args.eval_partial_reset, record_metrics=True)
    ...
    elif args.gen_small_model_strategy == 'target-mean':
    ...
    elif args.gen_small_model_strategy == 'target-single':
        target_eval_obs, _ = eval_envs.reset()
        sample_for_gen_small_model = {
    ...
    elif args.gen_small_model_strategy == 'random':
        rgb, depth, state = torch.rand((1, 128, 128, 3)), torch.rand((1, 128, 128, 1)), torch.rand((1, 42))
        sample_for_gen_small_model = {
            'rgb': rgb,
            'depth': depth,
    ...
    agent = generate_small_cnn_with_verify(
        agent, args.max_sparsity, sample_for_gen_small_model, lambda model, s: model(s)
    )
    # --------- use sample from source env to generate small model ----------
    ...
    for p in agent.parameters():
        p.requires_grad = True
    optimizer = optim.Adam(agent.parameters(), lr=args.learning_rate, eps=1e-5)
    print(f'compressed agent: {agent}')
    ...
    for iteration in range(start_iter_idx, args.num_iterations + 1):
        ...
        # if iteration < args.num_iterations / 3:
        # if global_step < 10000000:
        #     sparsity_list = np.linspace(0, args.max_sparsity, 3).tolist()[0: 1]
        # elif 10000000 <= global_step < 20000000:
        #     sparsity_list = np.linspace(0, args.max_sparsity, 3).tolist()[0: 2]
        # else:
        ...
        agent.eval()
        if iteration % args.eval_freq == 1 or args.eval_freq == 1:
            print("Evaluating")
            ...
            for test_sparsity in sparsity_list:
                ...
                set_sparsity(actor, test_sparsity)
        ...
        import tqdm
        for step in tqdm.tqdm(range(0, args.num_steps), desc='rollout...', leave=True, dynamic_ncols=True):
        ...
        pbar = tqdm.tqdm(total=args.update_epochs * (args.batch_size // args.minibatch_size), desc='updating...', leave=True, dynamic_ncols=True)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.target_kl is not None and approx_kl > args.target_kl:
                    # print(f'early stop due to high approx_kl ({approx_kl:.6f} > {args.target_kl:.6f})')
                    break
