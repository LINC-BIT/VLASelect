...
from train.vla_adapter_new.vla_rft.pretrain_world_model import DynamicsWorldModel, StateNormalizer


DEFAULT_MODEL_DIR = reference.DEFAULT_MODEL_DIR
DEFAULT_OUTPUT_DIR = "train/vla_adapter_new/vla_rft/outputs/online_rl"
DEFAULT_WORLD_MODEL_CHECKPOINT = "ckpt/vla_adapter_new/vla_rft/outputs/world_model/latest/best_world_model.pt"
DEFAULT_FBS_POLICY_CHECKPOINT = "ckpt/vla_adapter_new/ours/outputs/20260502-112804/best_policy.pt"
DEFAULT_SUMMARY_NAME = "vla_rft_training_summary.json"
...
class Args:
    ...
    wm_reward_weight: float = 0.4
    wm_success_weight: float = 0.4
    wm_reference_weight: float = 0.2
...
def collect_static_model_sample(args: Args, device: torch.device, env_id: str) -> Dict[str, Any]:
    sample_env = make_vector_env_for_env_id(args, device, env_id, 1, record_metrics=False)
    try:
        obs, _ = sample_env.reset(seed=args.seed)
...
def build_static_student_policy(args: Args, device: torch.device, env_id: str) -> reference.HandVLAAdapterActorCritic:
    materialized_fbs_metadata = build_materialized_fbs_metadata(
        args.fbs_policy_checkpoint,
    ...
    if fbs_policy is None:
        ...
        load_policy_state_from_checkpoint(args.fbs_policy_checkpoint, fbs_policy)
        maybe_persist_materialized_fbs_policy_to_checkpoint(
            args.fbs_policy_checkpoint,
            fbs_policy,
            expected_metadata=materialized_fbs_metadata,
    ...
    sample_batch = collect_static_model_sample(args, device, env_id)
    static_policy = generate_static_small_model(
        fbs_policy,
        device=device,
...
class WorldModelRewarder:
    ...
    def compute_reward(
        ...
        obs = {
            "rgb": rgbs.to(device=self.device),
        ...
        model_out = self.model.imagine_next(obs, action, self.normalizer)
        ...
        state_distance = torch.cdist(pred_state, self.reference_state).min(dim=1).values
        state_score = torch.exp(-state_distance / max(args.wm_state_temperature, 1e-6))
...
def train(args: Args) -> None:
    ...
    raw_policy = build_static_student_policy(args, device, current_env_id)
    ...
    if args.run_setup_smoke:
        reference.run_vla_inference_smoke(args, device, output_dir, policy=raw_policy)
    ...
    def maybe_switch_envs() -> Tuple[bool, bool, Optional[float]]:
        nonlocal envs, eval_envs, test_video_envs, next_obs, next_done, current_env_id, current_env_index
    ...
    for update in range(start_update, num_updates + 1):
        ...
        for step in range(args.num_steps):
            ...
            verified_reward, reward_info_last = rewarder.compute_reward(step_rgbs, step_states, action, args)
            ...
            if (
                ...
                print(
                    ...
                    f"mixed_reward_mean_so_far={reward_mean_so_far:.4f} elapsed_h={elapsed_hours:.2f}"
                )
        ...
        metrics_history.append(metric)
        print(
            f"[train] update={update}/{num_updates} env={current_env_id} "
            f"reward={metric['reward_mean']:.4f} env_reward={metric['env_reward_mean']:.4f} "
            f"verified_reward={metric['verified_reward_mean']:.4f} "
    ...
    save_summary(
        output_dir,
        {
            ...
            "envs_id": [current_env_id] if continual_env_schedule is None else continual_env_schedule.env_ids,
            "env_change_time_points": [] if continual_env_schedule is None else continual_env_schedule.change_time_points,
            "world_model_checkpoint": args.world_model_checkpoint,
            "fbs_policy_checkpoint": args.fbs_policy_checkpoint,
            "stop_reason": stop_reason,
            "stopped_early_zero_success": stopped_early_zero_success,
            "best_success_once": best_success_once,
            "num_metric_points": len(metrics_history),
            "final_eval_metrics": final_eval_metrics,
            "train_success_once_summary": reference.summarize_success_series(metrics_history, "train_success_once"),
            "train_success_at_end_summary": reference.summarize_success_series(metrics_history, "train_success_at_end"),
            "eval_success_once_summary": reference.summarize_success_series(metrics_history, "eval_success_once"),
