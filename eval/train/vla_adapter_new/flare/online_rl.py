...
DEFAULT_OUTPUT_DIR = "train/vla_adapter_new/flare/outputs"
...
class Args:
    ...
    head_learning_rate: float = 2e-5
    state_learning_rate: float = 2e-5
    value_head_learning_rate: float = 2e-5
    ...
    rollout_micro_batch_size: int = 128
...
class SeparateHandVLAAdapterActorCritic(nn.Module):
    def __init__(self, base_policy: reference.HandVLAAdapterActorCritic, device: torch.device) -> None:
        super().__init__()
        self.device = device
...
def build_optimizer(args: Args, policy: SeparateHandVLAAdapterActorCritic) -> optim.Optimizer:
    param_groups = []
...
def build_policy(args: Args, device: torch.device) -> SeparateHandVLAAdapterActorCritic:
    static_policy = build_static_student_policy(args, device)
    policy = SeparateHandVLAAdapterActorCritic(static_policy, device)
...
def train(args: Args) -> None:
    ...
    initial_eval_metrics = collect_training_policy_metric(envs, policy, args, reference) if use_train_success_only() else reference.evaluate_policy(policy, eval_envs, args.eval_episodes)
    initial_metric = {
        "update": 0,
        "global_step": global_step,
    ...
    for update in range(start_update, num_updates + 1):
        ...
        if use_train_success_only():
            memory_phase_tracker.mark("evaluation")
            eval_metrics = collect_training_policy_metric(envs, policy, args, reference)
            ...
            success_at_end = float(eval_metrics.get("train_success_at_end", eval_metrics.get("success_at_end", 0.0)))
            if success_once >= best_success_once:
                best_success_once = success_once
                save_training_checkpoint(output_dir / "best_policy.pt", policy, optimizer, update, global_step, best_success_once, max(best_success_at_end, success_at_end))
    ...
    memory_phase_tracker.mark("evaluation")
    final_eval_metrics = collect_training_policy_metric(envs, policy, args, reference) if use_train_success_only() else reference.evaluate_policy(policy, eval_envs, args.eval_episodes)
