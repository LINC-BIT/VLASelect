import os
os.environ["CUDA_VISIBLE_DEVICES"] = "2"
import sys
sys.path.append(os.getcwd())
import agents.jetcobot # imports your robot and registers it
import agents.dofbot_pro
import agents.mycobot_pro
import agents.dofbot_se
import agents.amazinghand
import agents.amazinghand_cpu
import my_empty_env
import envs.pick_obj_random
import envs.grasp_obj_random
import envs.fenduan
import torch
from dataclasses import dataclass
from typing import Annotated, Optional
import tyro
import numpy as np
import gymnasium as gym
import mani_skill
from mani_skill.agents.controllers.base_controller import DictController
from mani_skill.envs.sapien_env import BaseEnv
from mani_skill.utils.wrappers import RecordEpisode
from pathlib import Path
from envs.scan import UniformScanPolicy, PDJointPosScan
from PIL import Image


@dataclass
class Args:
    robot_uid: Annotated[str, tyro.conf.arg(aliases=["-r"])] = "amazinghand_right"
    sim_backend: Annotated[str, tyro.conf.arg(aliases=["-b"])] = "cpu"
    control_mode: Annotated[str, tyro.conf.arg(aliases=["-c"])] = "pd_joint_pos"
    keyframe: Annotated[Optional[str], tyro.conf.arg(aliases=["-k"])] = None
    shader: str = "default"
    keyframe_actions: bool = False
    random_actions: bool = False
    none_actions: bool = False
    zero_actions: bool = False
    sim_freq: int = 300
    control_freq: int = 20
    seed: Annotated[Optional[int], tyro.conf.arg(aliases=["-s"])] = None

def main(args: Args):
    # =========================
    # 1. 创建原始环境
    # =========================
    env_settings = dict(
        object_type="cube",                                     # "cube", "sphere", "cylinder", "box", 或者 None 表示随机选择
        object_size_info={                                      # 一个字典，预设物体尺寸参数 (m)，设置为 {} 表示随机选择
            'cube': {'half_size': 0.015},                        # cube 的边长为 0.06m
        },                                      
        object_mass=0.1,                                        # 物体质量 (kg)，None 表示随机选择
        object_color=[1, 0, 0, 1],                              # 物体颜色，输入一个 RGBA 列表，None 表示随机选择，各维度 0 ~ 1 之间
        randomize_camera=False,                                 # 是否随机摄像头位置, None 表示部分随机
        randomize_table=False,
        default_friction=True,
        randomize_skybox=False,
        randomize_disruptors=False,
        randomize_blur=False,
        default_lighting=True,
    )
    base_env = gym.make(
        # "MyEmpty-v1",
        "GraspObjectRandom-v1",
        # "Empty-v1",
        # "PickObjectRandomDofbot-v1",
        obs_mode="rgbd",
        reward_mode="none",
        enable_shadow=True,
        control_mode=args.control_mode,
        robot_uids=args.robot_uid,
        sensor_configs=dict(shader_pack=args.shader),
        human_render_camera_configs=dict(shader_pack=args.shader),
        viewer_camera_configs=dict(shader_pack=args.shader),
        render_mode="rgb_array",  # ⚠️ 录视频必须是 rgb_array
        # sim_config=dict(sim_freq=args.sim_freq, control_freq=args.control_freq),
        sim_backend=args.sim_backend,
        max_episode_steps=5000,
        # max_episode_steps=50,
        **env_settings
    )

    # =========================
    # 2. 用 RecordEpisode 包装
    # =========================
    video_dir = Path("videos")
    video_dir.mkdir(exist_ok=True)

    rec_env = RecordEpisode(
        base_env,
        output_dir=video_dir,
        save_trajectory=False,      # True 可保存 npz
        save_video=True,
        video_fps=args.control_freq,
        info_on_video=False
    )

    # =========================
    # 3. reset
    # =========================
    rec_env.reset(seed=0)
    env: BaseEnv = rec_env.unwrapped

    print(f"Selected robot {args.robot_uid}. Control mode: {args.control_mode}")
    print("Selected Robot has the following keyframes to view: ")
    print(env.agent.keyframes.keys())

    env.agent.robot.set_qpos(env.agent.robot.qpos * 0)

    kf = None
    # if len(env.agent.keyframes) > 0:
    #     kf_name = None
    #     if args.keyframe is not None:
    #         kf_name = args.keyframe
    #         kf = env.agent.keyframes[kf_name]
    #     else:
    #         for kf_name, kf in env.agent.keyframes.items():
    #             break

    #     if kf.qpos is not None:
    #         env.agent.robot.set_qpos(kf.qpos)
    #         env.agent.controller.reset()
    #     if kf.qvel is not None:
    #         env.agent.robot.set_qvel(kf.qvel)

    #     env.agent.robot.set_pose(kf.pose)
    #     if kf_name is not None:
    #         print(f"Viewing keyframe {kf_name}")

    if env.gpu_sim_enabled:
        env.scene._gpu_apply_all()
        env.scene.px.gpu_update_articulation_kinematics()
        env.scene._gpu_fetch_all()

    # =========================
    # 4. 主循环（录视频）
    # =========================
    step = 0
    stage = 0
    pre_action = np.zeros(6)
    p_f_rights = []
    p_p_rights =  []
    p_f_lefts =  []
    p_p_lefts = []
    PDJointDeltaScan_policy = PDJointPosScan(action_dim=env.action_space.shape[0])
    while True:
        print(f'step={step}')
        mins = np.array([-1.571, -1.571, -1.571, -1.571, -1.571, -1])
        maxs = np.array([1.571, 1.571, 1.571, 1.571, 3.142, 1])
        if args.random_actions:
            action = rec_env.action_space.sample()
            # action = 0.1 * torch.randn_like(action)
            # action[:-1] = 0
            # action[-1] = 1
        elif args.none_actions:
            action = None
        elif args.zero_actions:
            # action = np.array([0, -0.8, -1, 0])
            # action = np.ones(8)
            # action = torch.zeros(8).to('cuda')
            action = torch.tensor([2., 2., 2., 2., -2., -2., -2., -2.]).to('cuda')
            # print('a')
            # for i in range(4):
            #     action[i] = 1
            #     action[i + 4] = -1
            # action[-1] = 0.2
            # action = np.zeros(0.034, 1.002, 0.957, 0.428, 0, -1)
            # action = np.array([0, 0.9, 0.9, 0.5, 0, -1])
            # action = np.array([0, -0.5, -0.5, -0.5, 0, -1])

            # action = f(np.array([-1.571, -0.394, -0.710, -1.310, 0, 0.5]))
            # action = np.array([0, 0, 0, 0, 0, -1])
        elif args.keyframe_actions:
            assert kf is not None, "this robot has no keyframes"
            if isinstance(rec_env.agent.controller, DictController):
                action = rec_env.agent.controller.from_qpos(kf.qpos)
            else:
                action = kf.qpos
        else:
            action = None

        
        
        # p_f_right, p_p_right, p_f_left, p_p_left = rec_env.base_env.agent._get_gripper_constraint()

        # p_f_rights.append(p_f_right.unsqueeze(0))
        # p_p_rights.append(p_p_right.unsqueeze(0))
        # p_f_lefts.append(p_f_left.unsqueeze(0))
        # p_p_lefts.append(p_p_left.unsqueeze(0))

        step += 1

        # if step < 10:
        #     action[-1] = -1
        # elif step >= 10:
        #     action[-1] = 1

        print(rec_env.base_env.agent.get_proprioception())
        action = PDJointDeltaScan_policy.act()
        # print('action =', action)
        obs, reward, terminated, truncated, info = rec_env.step(action)
        gray_array:np.ndarray = obs['sensor_data']['base_camera']['depth'].cpu().numpy()[0]
        rgb_array:np.ndarray = obs['sensor_data']['base_camera']['rgb'].cpu().numpy()[0]

        gray_rgb = np.repeat(gray_array, 3, axis=2)
        gray_rgb = (gray_rgb / gray_rgb.max() * 255).astype(np.uint8)

        start, end = 32, 96
        center_gray = gray_rgb[:, start:end, :]  # 形状 (128, 64, 3)
        center_rgb = rgb_array[:, start:end, :]   # 形状 (128, 64, 3)


        # 3. 左右拼接 (各取一半)
        # gray 取左半边 [0:128, 0:64], rgb 取右半边 [0:128, 64:128]

        # 在宽度轴 (axis=1) 拼接
        combined_h = np.concatenate([center_rgb, center_gray], axis=1)

        Image.fromarray(combined_h).save(f'scan.png')
        #     if step % 20 < 10:
        #         action = np.array([0 - (step % 10) * 0.15, 0.6, 0.4, 0.1, 0, 0.6])
        #     else:
        #         action = np.array([-1.5 + (step % 10) * 0.15, 0.6, 0.4, 0.1, 0, 0.6])
        # env.agent.robot
        # pre_action = action

        if truncated:
            break
    
    # print("p_f_right =", torch.cat(p_f_rights, dim=0).mean(dim=0).cpu().tolist(), "std =", torch.cat(p_f_rights, dim=0).std(dim=0))
    # print("p_p_right =", torch.cat(p_p_rights, dim=0).mean(dim=0).cpu().tolist(), "std =", torch.cat(p_p_rights, dim=0).std(dim=0))
    # print("p_f_left =", torch.cat(p_f_lefts, dim=0).mean(dim=0).cpu().tolist(), "std =", torch.cat(p_f_lefts, dim=0).std(dim=0))
    # print("p_p_left =", torch.cat(p_p_lefts, dim=0).mean(dim=0).cpu().tolist(), "std =", torch.cat(p_p_lefts, dim=0).std(dim=0))

    # =========================
    # 5. 关闭并写文件
    # =========================
    rec_env.close()
    print(f"Video saved to: {video_dir}")
    # env.save_data('envs/regression_datas')

if __name__ == "__main__":
    main(tyro.cli(Args))
