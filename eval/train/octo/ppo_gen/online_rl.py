...
class Args:
    ...
    cuda: bool = True
    track: bool = False
    tracking_project_name: str = "EuroSys2026"
    tracking_entity: Optional[str] = None
    tracking_group: str = "ppo_gen"
...
class FlattenRGBDObservationWrapper2(gym.ObservationWrapper):
    ...
    def observation(self, observation: dict):
        ...
        goal_pos = _resolve_extra_tensor(
            ...
            default_dim=3,
            device=device,
            derive=lambda obs_extra: _slice_xyz(obs_extra["cubeB_pose"]) if "cubeB_pose" in obs_extra else None,
        )
...
def build_agent(args: Args, device: torch.device, env_kwargs: dict):
    if args.small_model_generation_strategy != "source":
        raise ValueError("ppo_gen baseline fixes small_model_generation_strategy to 'source'")
...
def make_env_kwargs(args: Args):
    with open(args.env_config_path, "r") as f:
        demo_info = json.load(f)
        env_kwargs = dict(demo_info["env_info"]["env_kwargs"])
...
def make_envs(args: Args, run_name: str, evaluate_only: bool):
...
def main():
    ...
    if args.exp_name is None:
        ...
        run_name = f"{args.env_id}/baselines/ppo_gen/{args.exp_name}/{datetime.now().strftime('%Y%m%d-%H%M%S')}"
    ...
    envs, eval_envs, env_kwargs = make_envs(args, run_name, args.evaluate)
    current_env_index = 0
    ...
    if not args.evaluate:
        if args.track:
            import local_tracking
            config = vars(args)
            config["env_cfg"] = dict(**env_kwargs, num_envs=args.num_envs, env_id=args.env_id, env_horizon=max_episode_steps)
            config["eval_env_cfg"] = dict(**env_kwargs, num_envs=args.num_eval_envs, env_id=args.env_id, env_horizon=max_episode_steps)
            if continual_env_schedule is not None:
                config["continual_envs_id"] = list(continual_env_schedule.env_ids)
            ...
            local_tracking.init(
                ...
                name=run_name,
                save_code=True,
                group=f"{args.tracking_group}/{args.env_id}",
            )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if should_stop_for_schedule:
            print(
                f"Reached continual schedule end at elapsed={elapsed_minutes:.2f} minutes after update, stopping."
            )
            break
        ...
        print(
            ...
            f"ppo_epochs={args.update_epochs}"
        )
