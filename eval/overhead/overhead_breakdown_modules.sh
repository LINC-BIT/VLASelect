...
MWE="${MWE:-0}"
BREAKDOWN_MODULES_ACTIVE_RUNTIME_SECONDS="${BREAKDOWN_MODULES_ACTIVE_RUNTIME_SECONDS:-120}"

if [[ -z "${MWE_WORKLOAD_RUNTIME_LIMIT_SECONDS+x}" ]]; then
    USER_SET_MWE_WORKLOAD_RUNTIME_LIMIT_SECONDS=0
    MWE_WORKLOAD_RUNTIME_LIMIT_SECONDS=$((BREAKDOWN_MODULES_ACTIVE_RUNTIME_SECONDS * 10))
else
...
vlaselect_resource_summary_start "overhead_breakdown_modules.sh"
...
launch_family_suite() {
    ...
    if [[ "$MWE" == "1" && "$USER_SET_MWE_WORKLOAD_RUNTIME_LIMIT_SECONDS" != "1" ]]; then
        ...
        family_mwe_workload_runtime_limit_seconds=$((selected_method_count * BREAKDOWN_MODULES_ACTIVE_RUNTIME_SECONDS))
    fi
