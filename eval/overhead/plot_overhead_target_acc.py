...
def collect_panel_metrics(panel, smoothing: float = 0.2, target_accuracy_override: float | None = None):
    suite_manifest_raw = panel.get('suite_manifest')
    ...
    target_accuracy = (
        float(target_accuracy_override)
        if target_accuracy_override is not None
        else max(value for _, value in vlaselect_series)
    )
    baseline_first_window_upper_bounds = []
    for method in methods:
        paper_name = PAPER_METHOD_BY_INTERNAL.get(method.get('name'))
