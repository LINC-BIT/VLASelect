...
LOG_FILE="$LOG_DIR/${TIME}_motivation_original_model.log"
mkdir -p "$LOG_DIR"
...
CUDA_VISIBLE_DEVICES=2 nohup python -u -m train.octo.ours_single_agent.motivation.original_model \
    ...
    --num_eval_envs 32 \
    --num_minibatches 16 \
    --tag motivation-original-model \
    > "$LOG_FILE" 2>&1 &

echo "Started original model training on GPU 2"
