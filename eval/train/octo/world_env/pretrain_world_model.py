...
def main():
    ...
    if args.exp_name is None:
        run_name = f"PickCube-v1/baselines/world_env/world_model/{datetime.now().strftime('%Y%m%d-%H%M%S')}"
    ...
    save_checkpoint(
        run_dir / "checkpoints" / "best_with_reference.pt",
        model,
        optimizer,
        args.epochs,
        final_metrics,
    ...
    print(
        f"finished training world model. best_val_loss={best_val_loss:.4f} "
        f"elapsed={elapsed / 60.0:.2f} min checkpoint={run_dir / 'checkpoints' / 'best_with_reference.pt'}"
    )
    writer.close()
