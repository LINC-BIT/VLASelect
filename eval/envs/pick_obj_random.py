...
from .pick_obj_cfgs import PICK_OBJ_CONFIGS
from .agents.dofbot_se import DOFBOT_SE, _quantize_qpos
...
PICK_OBJECT_DOC_STRING = """
**Task Description:** A task where the objective is to grasp a object with the {robot_id} robot 
and move it to a target goal position. Randomizations include object position, orientation, size, mass, color, 
...
@register_env("PickObjectRandom-v1", max_episode_steps=50)
class PickObjectEnv(BaseEnv):
    ...
    agent: Union[Panda, Fetch, XArm6Robotiq, SO100, WidowXAI, DOFBOT_SE]
    ...
    def __init__(
        ...
        **kwargs
        ...
        if robot_uids in PICK_OBJ_CONFIGS:
            cfg = PICK_OBJ_CONFIGS[robot_uids]
        else:
            cfg = PICK_OBJ_CONFIGS["panda"]
    ...
    def _load_agent(self, options: dict):
        if self.robot_uids == "dofbot_se":
            super()._load_agent(options, sapien.Pose(
                p=[-0.615, 0, 0.005], 
                q=matrix_to_quaternion(euler_angles_to_matrix(torch.tensor([0, 0, -np.pi/2]), convention="XYZ"))
            ))
            o_pose = sapien.Pose(
                p=[-0.615, 0, 0.005], 
                q=matrix_to_quaternion(euler_angles_to_matrix(torch.tensor([0, 0, -np.pi/2]), convention="XYZ"))
            )
            self.o_pose = np.concatenate([o_pose.p, o_pose.q], axis=-1)
        else:
    ...
    def _get_obs_sensor_data(self, apply_texture_transforms: bool = True) -> dict:
        ...
        for name, sensor in self.scene.sensors.items():
            if isinstance(sensor, Camera):
                ...
                if self.agent.distortion_params is not None and "rgb" in obs:
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        ...
        xy = rng.uniform(
            -self.object_spawn_radius,
            self.object_spawn_radius,
            size=(2,)
        )
        xyz[:, :2] = torch.from_numpy(xy).to(self.device)
        ...
        qs = random_quaternions(
            rng, lock_x=True, lock_y=True, device=self.device, bounds=(-np.pi / 18, np.pi / 18)
        )

        self.object.set_pose(Pose.create_from_pq(xyz, qs))
        # print("object", self.object.pose.p)
        # ---- random goal pose ----
        ...
        goal_xyz[:, :2] = torch.from_numpy(goal_xy).to(self.device)
        goal_xyz[:, 0] += self.object_spawn_center[0]
        goal_xyz[:, 1] += self.object_spawn_center[1]

        goal_z_offset = torch.from_numpy(rng.rand(1)).to(self.device).squeeze(-1)
        goal_xyz[:, 2] = goal_z_offset * self.max_goal_height + xyz[:, 2]

        self.goal_site.set_pose(Pose.create_from_pq(goal_xyz))
        # print("goal", self.goal_site.pose.p)
    ...
    def _get_obs_extra(self, info: dict):
        ...
        if "state" in self.obs_mode:
            obs.update(
                ...
                tcp_to_obj_pos=self.object.pose.p - self.agent.tcp_pose.p,
                obj_to_goal_pos=self.goal_site.pose.p - self.object.pose.p,
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: dict):
        tcp_to_obj_dist = torch.linalg.norm(self.object.pose.p - self.agent.tcp_pose.p, axis=1)
        ...
        obj_to_goal_dist = torch.linalg.norm(self.goal_site.pose.p - self.object.pose.p, axis=1)
...
PickObjectEnv.__doc__ = PICK_OBJECT_DOC_STRING.format(robot_id="panda")
...
class PickObjectEnvDofbot(PickObjectEnv):
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: dict):
        tcp_to_obj_dist = torch.linalg.norm(self.object.pose.p - self.agent.tcp_pose.p, axis=1)
        if self.prev_tcp_to_obj_dist is not None:
            dist_delta = tcp_to_obj_dist - self.prev_tcp_to_obj_dist
        else:
            dist_delta = tcp_to_obj_dist
        self.prev_tcp_to_obj_dist = tcp_to_obj_dist
        reaching_reward = 1 - torch.tanh(5 * tcp_to_obj_dist)
        punish = 0.5 * torch.clamp(dist_delta, min=0, max=1)
        # is_grasped = info["is_grasped"] * (tcp_to_obj_dist < torch.tensor(self.object_r, device=tcp_to_obj_dist.device) / 1.2)
        is_grasped = info["is_grasped"]
        reward = reaching_reward - punish * ~is_grasped
        # reward = reaching_reward
        reward += is_grasped * torch.exp(-1e2 * tcp_to_obj_dist)

        obj_to_goal_dist = torch.linalg.norm(self.goal_site.pose.p - self.object.pose.p, axis=1)
        # if torch.any(obj_to_goal_dist > 0.2):
        #     print("Object has been ejected (弹飞)")
        if self.prev_obj_to_goal_dist is not None:
            dist_delta = obj_to_goal_dist - self.prev_obj_to_goal_dist
        else:
            dist_delta = obj_to_goal_dist
        self.prev_obj_to_goal_dist = obj_to_goal_dist
        place_reward = 1 - torch.tanh(5 * obj_to_goal_dist)
        punish = 0.5 * torch.clamp(dist_delta, min=0, max=1)
        reward += (place_reward - punish * ~info["is_obj_placed"]) * is_grasped
        # reward += place_reward * is_grasped

        qvel = self.agent.robot.get_qvel()[..., :-6]
        static_reward = 1 - torch.tanh(5 * torch.linalg.norm(qvel, axis=1))
        place_weight = torch.sigmoid(-200 * (obj_to_goal_dist - 0.025))
        reward += static_reward * place_weight * is_grasped

        touching_table = self.agent.is_touching(self.table_scene.table, min_force=1)
        reward -= touching_table * 1

        reward[info["success"]] = 5. * is_grasped[info["success"]]
        reward[info["fail"]] = -3
