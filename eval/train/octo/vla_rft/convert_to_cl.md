...
`train/toy_cnn/vla_rft`文件夹下是基准方法vla_rft的实验代码，目前仅在单个Env（通过`--env-id`指定）上进行online RL。我现在想将其改为**多个Env连续出现**的场景，在多个Env上对同一个模型顺序进行online RL。
...
- 实现完成后，将你的实现过程以及修改前后代码的diff结果写入文档`train/toy_cnn/vla_rft/codex_run_logs/convert_to_cl/${DATETIME}.md`；
- 实现过程中你可以使用卡6验证运行效果；
- 你不能删除任何已有的文件；
- 你不需要做任何的git提交。
