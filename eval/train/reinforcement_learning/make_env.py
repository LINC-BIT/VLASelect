...
def make_eval_envs(env_id, num_envs: int, sim_backend: str, env_kwargs: dict, action_repeat: int = 1, video_dir: Optional[str] = None, wrappers: list[gym.Wrapper] = []):
    ...
    if sim_backend == "cpu":
        ...
        max_episode_steps = gym_utils.find_max_episode_steps_value(env) // action_repeat
        for wrapper in wrappers:
            env = wrapper(env)
        if video_dir:
